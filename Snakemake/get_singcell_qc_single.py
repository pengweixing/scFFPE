...
def get_qc(f_bam1,f_bam2,f_bam3,f_output,mapq):
    ...
    for i in cell_stats:
        total = math.ceil(cell_stats[i]['Total'])
        Duplicates = round(cell_stats[i]['Duplicates']/total,4)*100
        LowQ = int(cell_stats[i]['LowQ'])
        Unpaired = int(cell_stats[i]['Unpaired'])
        Final = math.ceil(cell_stats[i]['Final'])
        if i in number_FRiP:
            frags = number_FRiP[i]
        else:
            frags = 0
